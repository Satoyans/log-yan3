...
class commandLogSearchClass {
    ...
    static async logSearch(
        logMode: "blockcontainer" | "blockdestroy" | "blockplace" | "blockinteractedwith" | "signblockplace" | "all",
        playerName: string,
        pos: Vec3,
    ...
    private static makeForm(logs: (sdt.blockContainer | sdt.blockInteractedWith | sdt.blockDestroy | sdt.blockPlace | sdt.signBlockPlace)[], pos: Vec3) {
        ...
        if (logs.length === 0) {
            ...
            for (let data of logs.sort((a, b) => {
                return new Date(a.time).getTime() - new Date(b.time).getTime();
            })) {
                var timedata = new Date(data.time);
                var time = `${this.zero(timedata.getMonth() + 1)}/${this.zero(timedata.getDate())}/${this.zero(timedata.getHours())}:${this.zero(
                    timedata.getMinutes(),
                )}:${this.zero(timedata.getSeconds())}`;
