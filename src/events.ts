...
class events2emitDataClass {
    ...
    commandRegister() {
        command_1.command
            .register("log-yan", "log-yan setting command.", command_2.CommandPermissionLevel.Operator)
            .overload((params, origin, output) => {}, {
                select: command_1.command.enum("log-yan.select1", "logview"),
                toggle: [
                    command_1.command.enum("log-yan.view_on", "on"),
                    {
                        optional: false,
                ...
                mode: [
                    command_1.command.enum("log-yan.mode", "blockcontainer", "blockdestroy", "blockinteractedwith", "blockplace", "signblockplace", "all"),
                    { optional: false },
                ],
            })
            .overload((params, origin, output) => {}, {
                select: command_1.command.enum("log-yan.select1", "logview"),
                toggle: [
                    command_1.command.enum("log-yan.view_off", "off"),
                    {
                        optional: false,
                        options: command_2.CommandParameterOption.EnumAutocompleteExpansion,
                    },
            ...
            .overload((params, origin, output) => {}, {
                ...
                mode: [
                    command_1.command.enum("log-yan.setting", "logging_start", "logging_stop", "reload"),
                    {
                        optional: false,
                        options: command_2.CommandParameterOption.EnumAutocompleteExpansion,
                    },
        ...
        events.command.on((cmd, origin) => {
            ...
            if (splitcmd[1] === "logview") {
                if (splitcmd[2] == "on") {
                    ...
                    if (
                        logMode !== "blockcontainer" &&
                        logMode !== "blockdestroy" &&
                        logMode !== "blockinteractedwith" &&
                        logMode !== "blockplace" &&
                        logMode !== "all"
                    ) {
    ...
    reloadConfig() {
        ...
        if (Boolean(config.isRun)) {
            ...
            Object.keys(config.loggerEvent).forEach(key => {
                config.loggerEvent[key] = false;
            });
            this.loggerEvent = config.loggerEvent;
        }
    }
    ...
    itemUse(ev: entityEvent_1.ItemUseEvent) {
        ...
        if (!ev.player) return;
        if (
            !["lingering_potion", "splash_potion", "experience_bottle", "snowball", "ender_pearl", "ender_eye"].includes(
                ev.itemStack.getName().replace("minecraft:", ""),
            )
        )
            return;
    ...
    projectileShoot(ev: entityEvent_1.ProjectileShootEvent) {
        ...
        if (["Snowball", "Enderpearl", "LingeringPotion", "ThrownPotion", "ExperiencePotion", "ThrownEgg"].includes(ActorType[ev.projectile.getEntityTypeId()]))
            return;
    ...
    frameAttack() {
        const checkLoggerEvent = (): Boolean => {
            return this.loggerEvent["blockInteractedWith"];
        };
        const itemFrameBlock$attack = prochacker_1.procHacker.hooking(
            "?attack@ItemFrameBlock@@UEBA_NPEAVPlayer@@AEBVBlockPos@@@Z",
            nativetype_1.bool_t,
            { this: BlockActor },
            Player,
            BlockPos,
        )(onChangeItenFrameBlockAttack);
    ...
    blockContainer() {
        const checkLoggerEvent = (): Boolean => {
            return this.loggerEvent["blockContainer"];
        };
        const levelContainerModel$onItemChanged = prochacker_1.procHacker.hooking(
            "?_onItemChanged@LevelContainerModel@@MEAAXHAEBVItemStack@@0@Z",
            nativetype_1.void_t,
            null,
            core_1.StaticPointer,
            nativetype_1.int32_t,
            inventory_1.ItemStack,
            inventory_1.ItemStack,
            ...
            if (oldItem.amount === 0 && newItem.amount > 0) {
                //air => item
            ...
            } else if (oldItem.amount > 0 && newItem.amount === 0) {
                //item => air
                var emitData: edt.blockContainer = {
                    blockType: block.getName().replace("minecraft:", ""),
                    action: "remove",
                ...
                edq.blockContainer(emitData);
            } else if (oldItem.item.equalsptr(newItem.item)) {
                //Same item
    ...
    getElytra(ev: entityEvent_1.PlayerInventoryChangeEvent) {
        ...
        if (ev.newItemStack.getName() !== "minecraft:elytra" || ev.newItemStack.getCustomLore().length !== 0) return;
    ...
    signBlockPlace() {
        const checkLoggerEvent = (): Boolean => {
            return this.loggerEvent["signBlockPlace"];
        };
        ...
        function log(blockActor: BlockActor) {
            ...
            setTimeout(
                blockactor_ => {
                    ...
                    let after = savedTag;
                    if (before["BackText"]["Text"] !== after["BackText"]["Text"] || before["BackText"]["TextOwner"] !== after["BackText"]["TextOwner"]) {
                        //裏面が編集された
                    ...
                    if (before["FrontText"]["Text"] !== after["FrontText"]["Text"] || before["FrontText"]["TextOwner"] !== after["FrontText"]["TextOwner"]) {
                        //表面が編集された
        ...
        const signBlockActor$onUpdatePacket_ = prochacker_1.procHacker.hooking(
