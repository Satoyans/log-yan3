...
class emitDataQueryClass {
    static playerAttack(emitData: edt.playerAttack) {
        ...
        const victimId = emitData.victimId as string;
        const victimName = emitData.victimName;
        const dimensionId = DimensionId[emitData.dimensionId];
        query(
            `INSERT IGNORE INTO ${edt.queryText.playerAttack} ` +
    ...
    static entityDie(emitData: edt.entityDie) {
        ...
        const y = Math.ceil(pos.y);
        const z = Math.ceil(pos.z);
        const attackerId = emitData.attackerId as string;
        const attackerName = emitData.attackerName;
        const victimId = emitData.victimId as string;
        const victimName = emitData.victimName;
        const cause = emitData.cause as string;
        const dimensionId = DimensionId[emitData.dimensionId];
        query(
            `INSERT IGNORE INTO ${edt.queryText.entityDie} ` +
    ...
    static blockContainer(emitData: edt.blockContainer) {
        ...
        const dimensionId = DimensionId[emitData.dimensionId];
        query(
            `INSERT IGNORE INTO ${edt.queryText.blockContainer} ` +
    ...
    static blockInteractedWith(emitData: edt.blockInteractedWith) {
        ...
        const dimensionId = DimensionId[emitData.dimensionId];
        query(
            `INSERT IGNORE INTO ${edt.queryText.blockInteractedWith} ` +
    ...
    static itemThrow(emitData: edt.itemThrow) {
        ...
        const dimensionId = DimensionId[emitData.dimensionId];
        query(`INSERT IGNORE INTO ${edt.queryText.itemThrow} ` + `VALUES("${timestamp}","${playerName}","${itemName}","${x}","${y}","${z}","${dimensionId}")`);
    }
    static blockPlace(emitData: edt.blockPlace) {
        ...
        const dimensionId = DimensionId[emitData.dimensionId];
        query(
            `INSERT IGNORE INTO ${edt.queryText.blockPlace} ` +
                `VALUES("${timestamp}","${playerName}","${blockName}","${blockData}","${x}","${y}","${z}","${dimensionId}")`,
        );
    }
    static blockDestroy(emitData: edt.blockDestroy) {
        ...
        const dimensionId = DimensionId[emitData.dimensionId];
        query(
            `INSERT IGNORE INTO ${edt.queryText.blockDestroy} ` +
                `VALUES("${timestamp}","${playerName}","${blockName}","${blockData}","${x}","${y}","${z}","${dimensionId}")`,
        );
    }
    static lightningHitBlock(emitData: edt.lightningHitBlock) {
        ...
        const dimensionId = DimensionId[emitData.dimensionId];
        query(
            `INSERT IGNORE INTO ${edt.queryText.lightningHitBlock} ` + `VALUES("${timestamp}","${blockName}","${blockData}","${x}","${y}","${z}","${dimensionId}")`,
        );
    }
    static getElytra(emitData: edt.getElytra) {
        ...
        const playerName = emitData.playerName;
        const dimensionId = DimensionId[emitData.dimensionId];
        query(`INSERT IGNORE INTO ${edt.queryText.getElytra} ` + `VALUES("${timestamp}","${playerName}","${x}","${y}","${z}","${dimensionId}")`);
    }
    static signBlockPlace(emitData: edt.signBlockPlace) {
        ...
        query(
            `INSERT IGNORE INTO ${edt.queryText.signBlockPlace} ` +
